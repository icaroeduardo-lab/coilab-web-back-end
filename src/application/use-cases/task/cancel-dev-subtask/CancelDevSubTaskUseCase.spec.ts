import { CancelDevSubTaskUseCase } from './CancelDevSubTaskUseCase';
...
describe('CancelDevSubTaskUseCase', () => {
  it('closes all open issues and cancels subtask', async () => {
    ...
    const sut = new CancelDevSubTaskUseCase(repo, github);

    await sut.execute({ taskId: task.getId(), subTaskId, reason: 'Mudança de escopo' });
  ...
  it('calls GitHub updateIssue state=closed for each coilab-web open issue', async () => {
    ...
    await sut.execute({ taskId: task.getId(), subTaskId, reason: 'Cancelado' });
  ...
  it('throws when subtask is not typeId=4', async () => {
    ...
    repo.findById.mockResolvedValue(task);
    const sut = new CancelDevSubTaskUseCase(repo, makeGitHub());

    await expect(sut.execute({ taskId: task.getId(), subTaskId, reason: 'x' })).rejects.toThrow(
  ...
  it('throws when task not found', async () => {
    ...
    await expect(
      sut.execute({ taskId: randomUUID(), subTaskId: randomUUID(), reason: 'x' }),
  ...
  it('throws when subtask not found', async () => {
    ...
    await expect(
      sut.execute({ taskId: task.getId(), subTaskId: randomUUID(), reason: 'x' }),
