import { ITaskRepository } from '../../../../domain/repositories/ITaskRepository';
import { TaskId } from '../../../../domain/shared/entity-ids';

export interface UpdateDiscoveryFormInput {
  taskId: string;
  subTaskId: string;
  userId: string;
  fields: Record<string, unknown>;
}

export class UpdateDiscoveryFormUseCase {
  constructor(private readonly taskRepository: ITaskRepository) {}

  async execute(input: UpdateDiscoveryFormInput): Promise<void> {
    const task = await this.taskRepository.findById(TaskId(input.taskId));
    if (!task) throw new Error(`Task not found: ${input.taskId}`);

    task.assertEditable();

    const subTask = task.getSubTasks().find((s) => s.getId() === input.subTaskId);
    if (!subTask) throw new Error(`SubTask not found: ${input.subTaskId}`);

    const filledAt = new Date().toISOString();
    const existing = (subTask.getMetadata().form ?? {}) as Record<string, unknown>;
    const updatedFields: Record<string, unknown> = { ...existing };
    for (const [key, value] of Object.entries(input.fields)) {
      if (value !== undefined) {
        updatedFields[key] = { value, userId: input.userId, filledAt };
      }
    }

    subTask.updateMetadata({ form: updatedFields });

    await this.taskRepository.save(task);
  }
}
